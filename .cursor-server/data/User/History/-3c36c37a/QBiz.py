...
def initialize_model():
    ...
    conversation = ConversationChain(
        ...
        verbose=True,
        prompt=CaliforniaTopicTemplate.create_chat_messages()[0]["content"]
    )
...
def chat():
    try:
        ...
        if CaliforniaTopicTemplate.detect_off_topic(user_message):
            # Get appropriate redirection
            redirection = CaliforniaTopicTemplate.get_redirection(
                # Simple category detection - you can make this more sophisticated
                'default'
            )
            # Combine redirection with user's question
            response = conversation.predict(
                input=f"{user_message}\n\nAssistant: {redirection} "
            )
        else:
