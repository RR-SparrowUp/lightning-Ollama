...
def chat():
    try:
        ...
        if CaliforniaTopicTemplate.detect_off_topic(user_message):
            # Get appropriate redirection based on message content
            category = 'default'
            for topic, _ in CaliforniaTopicTemplate.TOPIC_REDIRECTS.items():
                if topic in user_message.lower():
                    category = topic
                    break
                    
            redirection = CaliforniaTopicTemplate.get_redirection(category)
            # Combine redirection with user's question
            full_prompt = f"{user_message}\n\nLet me redirect our conversation to California. {redirection}"
